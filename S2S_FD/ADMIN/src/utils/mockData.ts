...
const API_BASE = (process.env.REACT_APP_API_BASE as string) || 'http://localhost:5000/api';
...
export async function fetchIssues(params: { page?: number; limit?: number } = {}): Promise<Issue[]> {
  const qs = new URLSearchParams();
  if (params.page) qs.set('page', String(params.page));
  if (params.limit) qs.set('limit', String(params.limit));
  try {
    const payload = await safeFetchJson(`${API_BASE}/issues?${qs.toString()}`);
