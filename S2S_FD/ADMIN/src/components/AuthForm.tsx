...
import { login as apiLogin, register as apiRegister, clearToken } from '../utils/mockData';
import {
  Eye,
  EyeOff,
  Mail,
...
export function AuthForm({ onLogin }: AuthFormProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (loginData.email && loginData.password) {
        ...
        toast.error('Please enter valid administrator credentials');
      }
      setIsLoading(false);
    }, 1500);
  };

  // Real API-backed handlers (use these in forms)
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res = await apiRegister(registerData.fullName, registerData.email, registerData.password);
      if (res && res.token && res.user) {
        onLogin(res.user);
