...
import { mockCitizen, statusColors, categoryIcons, fetchIssues } from '../utils/mockData';
...
export function CitizenProfile() {
  ...
  React.useEffect(() => {
    let mounted = true;
    fetchIssues().then((data: any[]) => {
      if (!mounted) return;
      const normalized = data.map((i: any) => ({ ...i, createdAt: i.createdAt ? new Date(i.createdAt) : new Date() }));
      setUserIssues(normalized.filter((issue) => issue.reportedBy?.email === citizen.email));
    }).catch(() => {
      setUserIssues([]);
    });
